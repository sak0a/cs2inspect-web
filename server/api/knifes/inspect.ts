...
export default defineEventHandler(async (event) => {
    ...
    try {
        switch (action) {
            case 'create-url': {
                // Create an inspect URL for a knife
                const itemData: EconItem = {
                    defindex: body.defindex || WeaponType.KARAMBIT,
                    paintindex: body.paintindex || WeaponPaint.KARAMBIT_DOPPLER,
                    paintseed: body.paintseed || 387,
                    paintwear: body.paintwear || 0.01,
                ...
                Logger.success(`Created knife inspect URL`)
                return {
                    success: true,
                    inspectUrl,
                    itemData
            ...
            case 'inspect-item': {
                ...
                Logger.success(`Inspected ${analyzed.url_type} knife URL successfully`)
                return {
                    success: true,
            ...
            case 'decode-masked-only': {
                ...
                try {
                    ...
                    Logger.success(`Decoded masked knife URL offline`)
                    return {
                        success: true,
                        urlType: 'masked',
                        item: decodedItem,
            ...
            case 'decode-hex-data': {
                ...
                try {
                    ...
                    Logger.success(`Decoded knife hex data directly`)
                    return {
                        success: true,
                        item: decodedItem,
                        hexData: hexData,
            ...
            case 'validate-url': {
                // Validate a knife inspect URL with detailed analysis
                const inspectUrl = body.inspectUrl as string
                validateRequiredRequestData(inspectUrl, 'Inspect URL')
                ...
                try {
                    ...
                    Logger.info(`URL validation result: valid ${analyzed.url_type} knife URL`)
                    return {
                        success: true,
                        isValid: true,
                        urlType: analyzed.url_type,
                    ...
                    Logger.info(`URL validation result: invalid knife URL`)
                    return {
                        success: true,
                        isValid: false,
                        urlType: null,
            ...
            default:
                throw createError({
                    statusCode: 400,
                    message: `Unknown action: ${action}. Available actions: create-url, inspect-item, decode-masked-only, decode-hex-data, validate-url, client-status`
                });
        }

    } catch (error: any) {
        Logger.error(`Knife API Error: ${error.message}`)
