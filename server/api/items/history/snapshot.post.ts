...
import type { HistoryItemType, HistoryItemCategory, ItemHistorySnapshot } from '~/server/database/schema/itemHistory'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    Logger.success(`Created snapshot [${versionId}] for ${body.itemType} (defindex: ${body.defindex}, team: ${body.team})`)
    ...
    Logger.error(`Failed to create snapshot: ${error instanceof Error ? error.message : String(error)}`)
    throw createError({
      statusCode: 500,
