...
export const getLoadoutByName = async (
  steamId: string,
  name: string
): Promise<DBLoadout | undefined> => {
  const result = await db
    .select()
    .from(loadouts)
    .where(and(eq(loadouts.steamid, steamId), eq(loadouts.name, name.toLowerCase())))
    .limit(1)
  return result[0]
}
...
export const updateLoadoutByName = async (
  steamId: string,
  name: string,
  newName: string
): Promise<void> => {
...
export const deleteLoadout = async (id: string, steamId: string): Promise<void> => {
  ...
  await db
    .delete(pistols)
    .where(and(eq(pistols.loadoutid, loadoutId), eq(pistols.steamid, steamId)))
...
export const setActiveLoadout = async (id: string, steamId: string): Promise<void> => {
  ...
  await db.update(loadouts).set({ active: 0 }).where(eq(loadouts.steamid, steamId))
 ...
 */
export const duplicateLoadout = async (
  steamId: string,
  originalLoadoutId: string
): Promise<DBLoadout> => {
...
export const setShareCode = async (
  loadoutId: string,
  steamId: string,
  shareCode: string
): Promise<void> => {
...
export const getLoadoutByShareCode = async (shareCode: string): Promise<DBLoadout | undefined> => {
  const result = await db.select().from(loadouts).where(eq(loadouts.share_code, shareCode)).limit(1)
  return result[0]
}

export const setLoadoutAsDefault = async (loadoutId: string, steamId: string): Promise<void> => {
  // 1. Reset all defaults for this user
  await db.update(loadouts).set({ is_default: 0 }).where(eq(loadouts.steamid, steamId))
...
export const clearLoadoutItems = async (
  loadoutId: string,
  steamId: string,
  categories: string[] = []
): Promise<void> => {
  ...
  for (const category of tablesToClear) {
    ...
    if (table) {
      await db
        .delete(table)
        .where(and(eq(table.loadoutid, loadoutIdNum), eq(table.steamid, steamId)))
    }
  }
}
 ...
 */
export const importLoadoutFromShareCode = async (
  steamId: string,
  shareCode: string
): Promise<DBLoadout> => {
