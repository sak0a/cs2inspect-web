...
export async function countItemsByCategory(): Promise<ItemCategoryCount> {
    ...
    const [pistolCount, rifleCount, smgCount, heavyCount, knifeCount, gloveCount, agentCount, musicCount, pinCount] = await Promise.all([
    ...
    const weaponTotal = Number(pistolCount[0]?.count ?? 0) +
 ...
 */
export async function getWeaponBreakdown(): Promise<{ pistols: number; rifles: number; smgs: number; heavys: number }> {
...
export async function getTopUsersByLoadouts(limit: number = 10): Promise<TopUser[]> {
    ...
    const usersWithItems = await Promise.all(
        topLoadoutUsers.map(async (user) => {
            const [pistolCount, rifleCount, smgCount, heavyCount, knifeCount, gloveCount, agentCount, musicCount, pinCount] = await Promise.all([
                db.select({ count: sql<number>`COUNT(*)` }).from(pistols).where(eq(pistols.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(rifles).where(eq(rifles.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(smgs).where(eq(smgs.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(heavys).where(eq(heavys.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(knives).where(eq(knives.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(gloves).where(eq(gloves.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(agents).where(eq(agents.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(music).where(eq(music.steamid, user.steamId)),
                db.select({ count: sql<number>`COUNT(*)` }).from(pins).where(eq(pins.steamid, user.steamId))
            ])

            const totalItems = Number(pistolCount[0]?.count ?? 0) +
                Number(rifleCount[0]?.count ?? 0) +
                Number(smgCount[0]?.count ?? 0) +
                Number(heavyCount[0]?.count ?? 0) +
...
export async function getActivityTimeseries(days: number): Promise<ActivityTimeseriesEntry[]> {
    ...
    const itemsUpdatedPromises = [
        db.select({
            ...
            count: sql<number>`COUNT(*)`.as('count')
        }).from(pistols).where(gte(pistols.updated_at, cutoffDate)).groupBy(sql`DATE(${pistols.updated_at})`),
        db.select({
            date: sql<string>`DATE(${rifles.updated_at})`.as('date'),
            count: sql<number>`COUNT(*)`.as('count')
        }).from(rifles).where(gte(rifles.updated_at, cutoffDate)).groupBy(sql`DATE(${rifles.updated_at})`),
        db.select({
            date: sql<string>`DATE(${smgs.updated_at})`.as('date'),
            count: sql<number>`COUNT(*)`.as('count')
        }).from(smgs).where(gte(smgs.updated_at, cutoffDate)).groupBy(sql`DATE(${smgs.updated_at})`),
        db.select({
            date: sql<string>`DATE(${heavys.updated_at})`.as('date'),
            count: sql<number>`COUNT(*)`.as('count')
        }).from(heavys).where(gte(heavys.updated_at, cutoffDate)).groupBy(sql`DATE(${heavys.updated_at})`),
        db.select({
            date: sql<string>`DATE(${knives.updated_at})`.as('date'),
            count: sql<number>`COUNT(*)`.as('count')
        }).from(knives).where(gte(knives.updated_at, cutoffDate)).groupBy(sql`DATE(${knives.updated_at})`),
        db.select({
            date: sql<string>`DATE(${gloves.updated_at})`.as('date'),
            count: sql<number>`COUNT(*)`.as('count')
        }).from(gloves).where(gte(gloves.updated_at, cutoffDate)).groupBy(sql`DATE(${gloves.updated_at})`)
    ]
