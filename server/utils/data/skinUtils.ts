...
>(baseItem: T, databaseItem: U | undefined, skinsData: APISkin[]): APISkin | undefined {
  ...
  const matchingSkin = skinsData.find((skin) => {
    ...
    if (weaponMatch && paintMatch) {
      Logger.debug(`Match found weapon=${baseItem.weapon_name} skin=${skin.name}`, 'skin-match')
    }
...
export function filterSkins(
  ...
  return skins.filter((skin) => {
    ...
    // Weapon type filter
    if (criteria.weapon && !skin.weapon?.id.toLowerCase().includes(criteria.weapon.toLowerCase())) {
      return false
    }
