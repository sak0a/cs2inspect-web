...
describe('Zod schema validation', () => {
    describe('weaponSaveBodySchema', () => {
        ...
        it('should coerce string numbers to numbers', () => {
            const result = weaponSaveBodySchema.parse({
                ...validWeaponBody,
                paintseed: '123',
                paintindex: '38',
            })
        ...
        it('should reject negative paintindex', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintindex: -1 })
            ).toThrow()
        })

        it('should reject negative paintseed', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintseed: -1 })
            ).toThrow()
        })

        it('should reject paintwear out of range', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintwear: -0.1 })
            ).toThrow()
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintwear: 1.1 })
            ).toThrow()
        })

        it('should accept paintwear at boundaries', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintwear: 0 })
            ).not.toThrow()
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, paintwear: 1 })
            ).not.toThrow()
        })

        it('should reject non-boolean stattrak_enabled', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, stattrak_enabled: 'true' })
            ).toThrow()
        })

        it('should reject negative stattrak_count', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, stattrak_count: -1 })
            ).toThrow()
        })

        it('should accept null/undefined nametag', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, nametag: null })
            ).not.toThrow()
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, nametag: undefined })
            ).not.toThrow()
        })

        it('should reject nametag over 32 characters', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, nametag: 'A'.repeat(33) })
            ).toThrow()
        })

        it('should reject non-boolean active', () => {
            expect(() =>
                weaponSaveBodySchema.parse({ ...validWeaponBody, active: 'true' })
            ).toThrow()
        })
    })
    ...
    describe('resetRequestSchema', () => {
        it('should accept valid reset request', () => {
            expect(() =>
                resetRequestSchema.parse({ defindex: 7, team: 1, reset: true })
            ).not.toThrow()
        })
