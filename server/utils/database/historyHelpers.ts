...
function detectChangeType(
    ...
    if (oldSnapshot.paintwear !== newSnapshot.paintwear) {
        changes.push(
            `wear: ${oldSnapshot.paintwear.toFixed(4)} → ${newSnapshot.paintwear.toFixed(4)}`
        )
    }
    ...
    if (changes.length === 1) {
        if (
            ...
            changes[0]?.includes('StatTrak enabled') ||
            changes[0]?.includes('StatTrak disabled')
        )
    ...
    return {
        changeType,
        description:
            changes.length > 3 ? `${description}, +${changes.length - 3} more` : description,
    }
}
