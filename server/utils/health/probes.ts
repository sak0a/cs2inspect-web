...
async function calculateUptimePercentage(checkName: string, minutes: number = 60): Promise<number> {
    try {
        ...
        const rows = result as unknown as Array<{
            total_checks: number | bigint
            ok_checks: number | bigint
        }>
...
export async function checkDatabase(): Promise<HealthCheckResult> {
    ...
    try {
        ...
        const poolConfig = (
            pool as unknown as {
                pool?: { config?: { connectionLimit?: number }; _connectionQueue?: unknown[] }
            }
        ).pool
        result.metadata = {
...
export async function checkSteamClient(): Promise<HealthCheckResult> {
    ...
    try {
        const useSteamService = !!(
            process.env.STEAM_SERVICE_URL && process.env.STEAM_SERVICE_API_KEY
        )
        ...
        if (!stats.isAvailable) {
            ...
            if (!hasUsername && !hasPassword) {
                result.status = 'degraded'
                result.message =
                    'Steam client not configured (missing STEAM_USERNAME and STEAM_PASSWORD)'
            } else if (!hasUsername) {
                result.status = 'fail'
...
export async function checkImageProxy(): Promise<HealthCheckResult> {
    ...
    const sampleUrl =
        'https://steamcommunity-a.akamaihd.net/public/shared/images/header/logo_steam.svg'
    ...
    try {
        ...
        const ok =
            resp.status === 200 && (contentType.startsWith('image/') || contentType.includes('svg'))
        ...
        try {
            ...
            const ok =
                upstream.ok && (contentType.startsWith('image/') || contentType.includes('svg'))
            if (ok) {
                result.status = 'degraded'
                result.message =
                    'Upstream reachable, but local proxy call failed (using direct fetch)'
            } else {
                result.status = 'fail'
