...
describe('request logging helpers', () => {
  it('respects access logging toggles and always logs server errors', () => {
    expect(shouldLogAccess('/api/items/weapons', 200, {
      level: 'info',
      format: 'json',
    ...
    expect(shouldLogAccess('/api/health/live', 200, {
      level: 'info',
      format: 'json',
      logApiRequests: false,
      logHealthRequests: false,
    ...
    expect(shouldLogAccess('/api/items/weapons', 500, {
      level: 'info',
      format: 'json',
      logApiRequests: false,
      logHealthRequests: false,
  ...
  it('sets request correlation headers on success and error responses', async () => {
    ...
    app.use('/api/ok', eventHandler(() => ({ ok: true })))

    app.use('/api/fail', eventHandler(() => {
  ...
  it('creates request logger with Req tag by default', async () => {
    ...
    const app = createApp({
      onRequest: event => {
        ...
        const requestLogger = getRequestLogger(event) as unknown as { bindings?: () => Record<string, unknown> }
  ...
  it('includes requestId in standardized error response meta', async () => {
    ...
    app.use('/api/handled-error', eventHandler(withErrorHandling(async () => {
