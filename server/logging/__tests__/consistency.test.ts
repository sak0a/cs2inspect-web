    ...
    for (const relativePath of runtimeFiles) {
      ...
      for (const call of calls) {
        ...
        if (head && /^\[[A-Za-z0-9 _-]+\]/.test(head.trim())) {
          offenders.push(`${relativePath}:${source.getLineAndCharacterOfPosition(firstArg.getStart(source)).line + 1}`)
        }
      }
    }
    ...
    for (const relativePath of CONTEXT_REQUIRED_FILES) {
      ...
      for (const call of calls) {
        ...
        if (call.args.length < 2) {
          offenders.push(`${relativePath}:${source.getLineAndCharacterOfPosition(call.args.pos).line + 1}`)
          continue
        }

        const contextArg = call.args[1]
        if (contextArg && ts.isStringLiteralLike(contextArg) && contextArg.text.trim().length === 0) {
          offenders.push(`${relativePath}:${source.getLineAndCharacterOfPosition(contextArg.getStart(source)).line + 1}`)
        }
      }
    }
