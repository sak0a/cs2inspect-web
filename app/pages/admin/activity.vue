...
const columns: DataTableColumns<AdminActivityLogEntry> = [
  ...
  },
  {
    ...
    render(row) {
      ...
      const colorClass = actionColors[actionKey] || 'bg-gray-500/20 text-gray-400 border-gray-500/30'
  ...
  },
  {
    ...
    key: 'details',
    render(row) {
      if (!row.details || (typeof row.details === 'object' && Object.keys(row.details).length === 0)) {
        return h('span', { class: 'text-gray-500' }, '-')
      }
      let detailsText = ''
      try {
      ...
      <div class="table-container">
        ...
        <div v-if="!isLoading && activityLog.length === 0" class="empty-state">
          ...
          <p class="text-sm text-gray-500 mt-2">
            {{ actionFilter ? 'No activity matches the selected filter.' : 'No admin activity has been recorded yet.' }}
          </p>
        </div>
      </div>
