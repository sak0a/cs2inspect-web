    ...
    const resp = await $fetch<{ data: typeof fetchTestResult.value }>(
      '/api/admin/health/fetch-test',
      {
        ...
        body: {
          ...
          body:
            showBody.value && fetchTestBody.value.trim() ? fetchTestBody.value.trim() : undefined,
        },
      }
    )
  ...
  <div class="health-page">
    ...
    <div class="fetch-tester-section">
      ...
      <p class="text-xs text-white/35 mb-4">
        Send HTTP requests from the server process to test connectivity. Runs in the same network
        context as the app.
      </p>
