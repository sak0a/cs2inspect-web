...
const handleAgentTypeChange = async (team: 't' | 'ct', agentDefindex: number) => {
    ...
    await $fetch(
        ...
        .then(async () => {
            ...
            if (team === 't') {
                loadoutStore.selectedLoadout.selected_agent_t =
                    agentDefindex === -1 ? null : agentDefindex
            } else {
                loadoutStore.selectedLoadout.selected_agent_ct =
                    agentDefindex === -1 ? null : agentDefindex
            }
                    ...
                    >
                        <template v-if="ctAgents && ctAgents.length > 0">
                            <AgentTabs
                                ...
                                :agent="agent"
                                :is-selected="
                                    ctAgentType === parseInt(agent.id.replace('agent-', ''))
                                "
                                @select="handleAgentSelect"
                            />
                        </template>
                        <p v-else class="text-gray-400 py-4">
                            No Counter-Terrorist agents available
                        </p>
                    </div>
                    ...
                    >
                        <template v-if="tAgents && tAgents.length > 0">
                            <AgentTabs
                                ...
                                :agent="agent"
                                :is-selected="
                                    tAgentType === parseInt(agent.id.replace('agent-', ''))
                                "
                                @select="handleAgentSelect"
                            />
                        </template>
