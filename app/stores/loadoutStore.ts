    ...
    async fetchLoadouts(steamId: SteamId) {
      ...
      const promise = (async () => {
        ...
        try {
          ...
          const loadouts =
            response.data?.loadouts || (Array.isArray(response.data) ? response.data : [])
          if (!loadouts) {
          ...
          if (!this.selectedLoadoutId && this.loadouts.length > 0) {
            ...
            const activeLoadout = this.loadouts.find(
              (loadout: DBLoadout) => loadout.active === true || Number(loadout.active) === 1
            )
          ...
          this.error =
            'Failed to fetch loadouts: ' + (error instanceof Error ? error.message : String(error))
        } finally {
          this.isLoading = false
