...
export const useAdminStore = defineStore('admin', {
  ...
  getters: {
    ...
    totalItems: (state) => {
      ...
      return (
        items.weapons + items.knives + items.gloves + items.agents + items.musicKits + items.pins
      )
    },
  },
  ...
  actions: {
    ...
    async fetchTopUsers(limit = 10): Promise<void> {
      try {
        const response = await api.get<{ topUsers: AdminTopUser[] }>('/api/admin/stats/users', {
          limit,
        })
    ...
    async fetchUsers(
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          this.users = items
          this.usersTotal =
            response.pagination?.totalItems || pagination?.totalItems || items.length || 0
    ...
    async fetchActivityLog(
      ...
      const logFresh =
        !!this.lastFetch.activityLog && Date.now() - this.lastFetch.activityLog < CACHE_DURATION
      if (
        !force &&
        logFresh &&
