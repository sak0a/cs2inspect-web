...
interface RarityInfo { id: string; name: string; color: string }
...
export function useFilterSort<T extends FilterableItem>(options: UseFilterSortOptions<T>) {
  ...
  const availableRarities = computed<RarityInfo[]>(() => {
    ...
    return Array.from(map.values()).sort((a, b) => stickerRarityRank(a.id) - stickerRarityRank(b.id))
  })
  ...
  const filteredItems = computed<T[]>(() => {
    ...
    return items.value.filter((item) => {
      ...
      if (useEffectFilter && allowedEffects && !allowedEffects.has(item.effect || 'Other')) return false
      return true
    })
  })
