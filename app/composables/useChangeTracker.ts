 ...
 */
function hasStatTrak(
    config: TrackedConfiguration
): config is WeaponConfiguration | KnifeConfiguration {
...
export function useChangeTracker() {
    ...
    function detectChanges(
        ...
        if (hasStatTrak(newState)) {
            ...
            if (oldStatTrak.stattrak_enabled !== newState.stattrak_enabled) {
                changes.push({
                    ...
                    description: newState.stattrak_enabled
                        ? 'Enabled StatTrak'
                        : 'Disabled StatTrak',
                })
            }
        ...
        if (hasStickers(newState)) {
            const oldStickers =
                (oldState as { stickers?: Array<StickerConfiguration | null> }).stickers || []
