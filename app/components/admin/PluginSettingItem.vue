...
const emit = defineEmits<{
  (e: 'save', payload: { key: string; value: string | number | boolean | Record<string, unknown> | unknown[] }): void
}>()
    ...
    <div class="flex flex-col gap-3 p-4">
      ...
      <div v-if="!isEditing" class="value-display">
        ...
        <template v-else-if="setting.type === 'json'">
          <pre class="text-xs bg-gray-800/50 px-3 py-2 rounded-lg text-gray-300 overflow-x-auto max-h-32">{{ displayValue }}</pre>
        </template>
        <template v-else>
      ...
      <div v-else class="edit-area">
        ...
        <template v-else-if="setting.type === 'json'">
          <NInput
            ...
            @update:value="(val: string) => { editValue = val; jsonError = null }"
          />
