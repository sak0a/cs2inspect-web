...
const chartData = computed(() => {
  ...
  const labels = topUsers.value.map((user, index) => {
    ...
    const truncated = steamId.length > 12
      ? `${steamId.slice(0, 4)}...${steamId.slice(-4)}`
      : steamId;
    ...
    datasets: [
      {
        ...
        backgroundColor: topUsers.value.map((_, i) => rankColors[i] ?? rankColors[rankColors.length - 1] ?? 'rgba(59, 130, 246, 0.4)'),
        borderColor: topUsers.value.map((_, i) => {
          const color = rankColors[i] ?? rankColors[rankColors.length - 1] ?? 'rgba(59, 130, 246, 0.4)';
