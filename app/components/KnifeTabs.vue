...
const handleDefaultWeaponClick = (team: number): void => {
    try {
        ...
        const errorMessage =
            error instanceof Error ? error.message : 'Failed to select default knife'
    ...
    <div v-if="weaponData.availableTeams === 'both'">
        ...
        >
            <NTabPane name="ct" :tab="String(t('teams.counterTerrorists'))">
                ...
                <NCard
                    v-if="
                        !weaponData.weapons.some((w: KnifeItemData) => w.databaseInfo?.team === 2)
                    "
                    :style="{
                        borderColor: '#313030',
                        background:
                ...
                <NCard
                    v-for="weapon in weaponData.weapons.filter(
                        (w: KnifeItemData) => w.databaseInfo?.team === 2
                    )"
            ...
            <NTabPane name="t" :tab="String(t('teams.terrorists'))">
                ...
                <NCard
                    v-if="
                        !weaponData.weapons.some((w: KnifeItemData) => w.databaseInfo?.team === 1)
                    "
                    :style="{
                        borderColor: '#313030',
                        background:
                ...
                <NCard
                    v-for="weapon in weaponData.weapons.filter(
                        (w: KnifeItemData) => w.databaseInfo?.team === 1
                    )"
                    :key="`${weapon.id}-${weapon.databaseInfo?.paintindex || 0}`"
                    :style="{
                        borderColor: weapon.rarity?.color || '#313030',
