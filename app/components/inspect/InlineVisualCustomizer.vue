...
const updateSelectedElementOffsetExternal = (axis: 'x' | 'y', value: number | null) => {
  ...
  const v = typeof value === 'number' ? value : 0
  const def =
    el.type === 'sticker' ? getStickerDefaultCanvasPos(el) : normalizedToCanvasInImage(el.position)
  ...
  for (const element of sorted) {
    ...
    if (element.apiData?.image) {
      ...
      if (cachedImg && cachedImg.complete && cachedImg.naturalWidth > 0) {
        ...
        const fallbackWidth =
          (element.type === 'sticker' ? STICKER_MAX_WIDTH_PX.value : KEYCHAIN_MAX_WIDTH_PX.value) *
          element.scale *
          globalScale
      ...
      const fallbackWidth =
        ...
        globalScale
      const fallbackHeight =
        (element.type === 'sticker' ? STICKER_MAX_HEIGHT_PX.value : KEYCHAIN_MAX_HEIGHT_PX.value) *
        element.scale *
        globalScale
      bounds = {
        left: pos.x - fallbackWidth / 2,
      ...
      <div class="flex-none flex items-center p-1 max-w-[70%]">
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <span class="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-px"
              >Scale</span
            >
