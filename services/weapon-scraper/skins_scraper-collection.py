#!/usr/bin/env python3
"""
CS:GO Collection Skins Video Scraper
Scrapes weapon skin videos from collections with threading and rate limiting.
...
Usage:
    ...
    python skins_scraper-collection.py -c the-2021-dust-2-collection --workers 3 --delay 2.0
...
class CSGOCollectionScraper:
    def __init__(self, collection: str = "the-harlequin-collection", max_workers: int = 5, delay: float = 1.0, skip_tracked: bool = True):
        """
        Initialize the scraper for collection items

        Args:
            collection: Collection name (e.g., "the-harlequin-collection")
        ...
        self.base_url = f"https://{os.getenv('BASE_URL', 'exampleskins.gg')}"
        self.collection_url = f"{self.base_url}/collections/{collection}"
    ...
    def get_skin_links(self) -> List[str]:
        """
        Get all skin links from the collection page

        Returns:
            List of skin URLs
        """
        logger.info(f"Fetching skin links from {self.collection_url}")

        response = self.rate_limited_request(self.collection_url)
        if not response:
            logger.error(f"Failed to fetch collection page: {self.collection_url}")
            return []

        soup = BeautifulSoup(response.content, 'html.parser')

        # Find all skin links using the provided CSS selector
        # Adjusted selector to be more flexible for collection pages
        skin_links = []
    ...
    def scrape_all(self) -> None:
        ...
        logger.info(f"Starting scrape for collection: {self.collection}")
...
def main():
    parser = argparse.ArgumentParser(description='CS:GO Collection Skins Video Scraper')
    parser.add_argument('--collection', '-c', default='the-harlequin-collection',
                        help='Collection name (e.g., the-harlequin-collection, the-2021-dust-2-collection)')
