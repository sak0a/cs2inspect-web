        ...
        <div class="toggle-list">
          <div v-for="(value, key) in features" :key="key" class="toggle-row">
            <label class="toggle-switch">
              <input type="checkbox" v-model="features[key as keyof typeof features]" />
              <span class="toggle-slider" />
            </label>
            <div class="toggle-info">
              <span class="toggle-name">{{ key }}</span>
              <span class="toggle-desc">{{ featureDescriptions[key as string] }}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="weapon-list">
            <div v-for="(w, i) in weaponCommands.weapons" :key="i" class="weapon-entry">
              <div class="weapon-fields">
                ...
                <div class="form-group">
                  <label>Command</label>
                  <input v-model="w.command" type="text" :placeholder="w.key || 'awp'" />
                </div>
              ...
              <button class="btn-remove" @click="removeWeapon(i)" title="Remove">&#215;</button>
            </div>
          </div>
      ...
      <div v-if="currentStep === 4" class="step-panel">
        ...
        <div class="toggle-row" style="margin-top: 1rem">
          ...
          <div class="toggle-info">
            ...
            <span class="toggle-desc">Write logs to disk (recommended for production)</span>
          </div>
        </div>
      </div>
