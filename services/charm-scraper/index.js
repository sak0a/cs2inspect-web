...
async function scrapeStickerSlabs() {
    ...
    for (let id = startId; id <= END_ID; id += BATCH_SIZE) {
        ...
        if (consecutiveErrors > 20) {
            // Increased threshold for batching
            console.error(
                'Too many consecutive errors. Stopping to prevent IP ban or useless retries.'
            )
            process.exit(1)
        }
        ...
        fs.writeFileSync(
            PROGRESS_FILE,
            JSON.stringify({ lastId: currentBatchEnd, timestamp: new Date().toISOString() })
        )

        // Log progress
        console.log(
            `Stickers processed: ${currentBatchEnd}/${END_ID} (Downloaded this session: ${totalDownloaded})`
        )
