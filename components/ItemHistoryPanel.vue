...
const handleRestore = async () => {
  ...
  try {
    const response = await $fetch<{ success: boolean; message: string }>('/api/items/history/restore', {
      method: 'POST',
      body: {
  ...
  >
    <NDrawerContent :title="String(t('history.title'))" closable class="history-drawer-content">
      <div class="history-panel">
        <!-- Loading State -->
        <div v-if="state.isLoading && state.records.length === 0" class="flex justify-center items-center py-8">
          <NSpin size="large" />
        </div>
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between gap-2">
                  ...
                  >
                    ...
                    <div class="max-w-xs">
                      <div class="font-medium text-sm mb-1">{{ formatChangeType(record.change_type) }}</div>
                      <div class="text-xs text-gray-300">{{ record.change_description }}</div>
                    </div>
                  </NTooltip>
      ...
      >
        ...
        <div v-if="state.selectedRecord" class="mt-4 p-4 rounded-xl bg-black/40 border border-amber-500/20 backdrop-blur-sm">
          <div class="flex items-center gap-2 mb-2">
            <span class="font-mono text-amber-400 font-medium text-sm">{{ state.selectedRecord.version_id }}</span>
            <span class="text-xs text-gray-500">{{ formatDate(state.selectedRecord.created_at) }}</span>
          </div>
          <p v-if="state.selectedRecord.change_description" class="text-xs text-gray-400 leading-relaxed">
...
.version-id {
  ...
  text-shadow: 0 1px 3px rgba(0, 0, 0, 0.4), 0 0 8px rgba(245, 158, 11, 0.15);
}

.version-id:hover {
  @apply text-amber-300;
  text-shadow: 0 1px 3px rgba(0, 0, 0, 0.4), 0 0 12px rgba(245, 158, 11, 0.25);
}
