...
import {NModal, NInput, NPagination, NCard, NSpin, NSpace, NInputNumber, NButton, useMessage, NSwitch} from 'naive-ui'
...
const fetchSkinsForGloves = async () => {
...
const handleDuplicate = async () => {
  ...
  try {
    ...
    console.error('Error duplicating gloves:', error)
  } finally {
...
watch(() => props.weapon, () => {
  if (props.visible && props.weapon) {
    ...
    const dbInfo = props.weapon.databaseInfo as {
      defindex: number,
      active: boolean,
      paintIndex: number,
      paintWear: number,
