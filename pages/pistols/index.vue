...
const handleSkinSelect = async (skin: EnhancedWeaponResponse, customization: WeaponCustomization) => {
  ...
  try {
    const response = await fetch(`/api/weapons/save?steamId=${user.value.steamId}&loadoutId=${loadoutStore.selectedLoadoutId}&type=pistols`, {
      method: 'POST',
      headers: {
...
const fetchLoadoutSkins = async () => {
  ...
  try {
    ...
    await loadoutStore.fetchLoadoutWeaponSkins("pistols", user.value.steamId)
  ...
  <div class="p-4 bg-[#181818]">
    <div class="max-w-7xl mx-auto">
      ...
      <div v-else-if="!loadoutStore.selectedLoadoutId" class="text-center py-12">
        <p class="text-gray-400 mb-4">Please select or create a loadout to view pistols</p>
      ...
      <!-- Skins Grid -->
      <div v-else class=" grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2">
        <WeaponTabs class=""
