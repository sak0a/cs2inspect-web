...
const handleSkinSelect = async (skin: EnhancedWeaponResponse, customization: WeaponCustomization) => {
  ...
  try {
    const response = await fetch(`/api/weapons/save?steamId=${user.value.steamId}&loadoutId=${loadoutStore.selectedLoadoutId}&type=smgs`, {
      method: 'POST',
      headers: {
...
const fetchLoadoutSkins = async () => {
  ...
  try {
    ...
    await loadoutStore.fetchLoadoutWeaponSkins("smgs", user.value.steamId)
...
watch(() => showSkinModal.value, (isVisible) => {
  //console.log('SMGs - Skin Modal visibility changed:', isVisible, 'Selected weapon:', selectedWeapon.value)
  ...
  <div class="p-4 bg-[#181818]">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex justify-between mb-2">
        <h1 class="text-2xl font-bold text-white">SMGs</h1>
      ...
      <div v-else-if="!loadoutStore.selectedLoadoutId" class="text-center py-12">
        <p class="text-gray-400 mb-4">Please select or create a loadout to view smgs</p>
      ...
      <!-- Skins Grid -->
      <div v-else class=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-2">
        <WeaponTabs class=""
