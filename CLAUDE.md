...
- **Dev mock auth**: AI agents cannot use Steam OpenID in a browser. Enable `DEV_AUTH_ENABLED=true` in `.env` **for local development only — never in production**, then `POST /api/auth/dev/login` or `bun run cli dev:login`. See **`AGENTS.md`** for the full workflow, default SteamIDs, and curl examples.
